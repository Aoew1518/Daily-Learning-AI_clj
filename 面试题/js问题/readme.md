...
# 10. 说说js中的继承
- 是什么：在js中继承指的是让一个子类可以访问父类的属性和方法，有两种方式：
...
- 原型链继承：
    1. 子类原型对象===父类原型对象，子类实例对象===父类实例对象，子类实例对象的隐式原型===父类实例对象的显示原型，子类实例对象的显示原型===父类实例对象的显示原型，子类实例对象的显示原型的隐式原型===Object.prototype
    2. 缺点：
        1. 无法实现多继承
        2. 无法实现构造函数的复用
        3. 每个子类实例对象都会在显示原型上创建一份多余的属性
