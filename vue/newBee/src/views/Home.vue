...
nextTick(() => { // 写在nextTick中的逻辑一定会在页面加载完毕后执行，异步函数
    document.body.addEventListener('scroll', function () {
        this.scrollTop > 100 ? state.headerActive = true : state.headerActive = false
        // console.log(this.scrollTop); // 获取一个元素的内容滚动的像素距离
        // console.log(state.headerActive);
    })
})
</script>
...
.home {
    ...
    .category-list {
        //网格布局
        display: grid;
        grid-template-columns: 1fr 1fr 1fr 1fr 1fr; //列宽 需要几列写几个
        // grid-template-rows: repeat(4, 1fr); //行高
